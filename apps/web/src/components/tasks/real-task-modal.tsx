...
type ActivityEntry = { details?: any; details?: any;
  id: Id;
  action: string;
  user_name: string | null;
  created_at: string;
                    ...
                    task.activity.map((entry) => (
                      <div key={String(entry.id)} className="rounded-xl border bg-white p-3">
                        <p className="text-sm text-slate-700">
                          ...
                          {entry.action === "task_assignees_updated" && entry.details?.assignee_names ? `assigned task to ${entry.details.assignee_names.join(", ")}${entry.details.due_date ? ` (Due: ${entry.details.due_date})` : ""}` : entry.action.replaceAll("_", " ")}
                        </p>
                        <p className="mt-1 text-[11px] text-slate-400">
                          {formatDate(entry.created_at)}
                        </p>
