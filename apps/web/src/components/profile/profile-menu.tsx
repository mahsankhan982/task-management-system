...
import { Camera, Settings, LogOut, Layers, Mail, UserRound, KeyRound } from "lucide-react";
...
export default function ProfileMenu({ onLogout }: { onLogout: () => void }) {
  ...
  const [editor, setEditor] = useState<"photo" | "name" | "email" | "settings" | "password" | null>(null);
  useEffect(() => {
  ...
  function openEditor(mode: "photo" | "name" | "email" | "settings" | "password") { if (menu.current) menu.current.open = false; setEditor(mode); }
  return <>
    <details ref={menu} className="profile-menu relative shrink-0">
      ...
      <div className="profile-panel absolute right-0 top-12 z-[160] w-72 max-w-[calc(100vw-2rem)] rounded-2xl border p-4 shadow-2xl">
        ...
        <Link href="/dashboard/profile" onClick={() => { if (menu.current) menu.current.open = false; }}><UserRound size={17} /> View Profile</Link>
        <button type="button" onClick={() => openEditor("photo")}><Camera size={17} /> Change Profile Picture</button>
        <button type="button" onClick={() => openEditor("name")}><UserRound size={17} /> Edit Name</button>
        <button type="button" onClick={() => openEditor("email")}><Mail size={17} /> Edit Email</button>
        ...
        <button type="button" onClick={() => openEditor("password")}><KeyRound size={17} /> Change Password</button>
    ...
    <dialog ref={dialog} className="profile-editor" aria-labelledby="profile-editor-title" onClose={() => { setEditor(null); trigger.current?.focus(); }}>
      <h2 id="profile-editor-title" className="text-xl font-semibold">{editor === "password" ? "Change Password" : "Profile Settings"}</h2>
      {editor === "password" ? <ChangePasswordForm email={user.email} onCancel={() => dialog.current?.close()} /> : editor && <ProfileForm focusField={editor === "settings" ? undefined : editor} onCancel={() => dialog.current?.close()} />}
    </dialog>
  </>;
}
