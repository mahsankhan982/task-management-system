...
import { Camera, Settings, LogOut, Layers, Mail, UserRound } from "lucide-react";
...
export default function ProfileMenu({ onLogout }: { onLogout: () => void }) {
  ...
  const [editor, setEditor] = useState<"photo" | "name" | "email" | "settings" | null>(null);
  useEffect(() => {
  ...
  function openEditor(mode: "photo" | "name" | "email" | "settings") { if (menu.current) menu.current.open = false; setEditor(mode); }
  return <>
    ...
    <dialog ref={dialog} className="profile-editor" aria-labelledby="profile-editor-title" onClose={() => { setEditor(null); trigger.current?.focus(); }}>
      ...
      {editor && <ProfileForm focusField={editor === "settings" ? undefined : editor} onCancel={() => dialog.current?.close()} />}
    </dialog>
  </>;
}
