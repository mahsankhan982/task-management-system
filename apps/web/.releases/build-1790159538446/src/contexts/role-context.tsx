...
type RoleContextValue = {
  ...
  updateProfile: (profile: Pick<AuthUser, "full_name" | "email">) => void;
};
  ...
  return (
    <RoleContext.Provider
      value={{ user, role, setRole, permissions: getPermissions(role),
        updateProfile: (profile) => setUser(current => current ? { ...current, full_name: profile.full_name, email: profile.email } : current),
        updateAvatar: (url) => setUser(current => current ? { ...current, avatar_url: url } : current),
      }}
    >
      {children}
    </RoleContext.Provider>
