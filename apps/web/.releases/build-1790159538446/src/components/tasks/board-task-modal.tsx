...
import { canEditTaskDueDate } from "@/lib/permissions";
      ...
      <section className="min-h-0 overflow-y-auto p-6">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-xs font-semibold uppercase tracking-wide text-slate-400">Due date
            <input disabled={!permissions.editTask || !canEditTaskDueDate(role)} value={due} onChange={e=>setDue(e.target.value)} className="mt-2 h-10 w-full rounded-lg border border-slate-200 px-3 text-sm text-slate-700"/>
          </label>
        </div>
