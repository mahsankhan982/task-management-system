...
function readMeta() {
  ...
  return { ...data, defaultVersion: data.defaultVersion || data.versions[0].version, versions: data.versions };
}

function promoteRelease(version) {
  const current = readMeta();
  if (!current.versions.some(item => item.version === version)) throw new Error("Release no longer available");
  current.defaultVersion = version;
  current.currentVersion = version;
  const temporary = metaFile + "." + process.pid + ".tmp";
  fs.writeFileSync(temporary, JSON.stringify(current, null, 2) + "\n", "utf8");
  fs.renameSync(temporary, metaFile);
}
    ...
    const request = http.get({ hostname: "127.0.0.1", port, path: "/version.json", timeout: 1000 }, response => {
      ...
      response.on("end", () => {
        try {
          ...
          resolve(response.statusCode === 200 && (manifest.releaseId === version || manifest.version === version));
        } catch { resolve(false); }
      });
    });
...
function ensureRelease(item) {
  ...
  const ready = (async () => {
    ...
    while (Date.now() < deadline && !failed) {
      if (await probe(port, item.version)) { ports.set(item.version, port); return port; }
...
async function proxy(req, res, item, setCookie) {
  ...
  const upstream = http.request({
    ...
    headers["x-task-manager-release"] = item.version;
...
const gateway = http.createServer(async (req, res) => {
  try {
    ...
    // Serve the maintained worker even to pinned old releases. It fetches the
    // pinned release from the gateway rather than overriding it with cached HTML.
    if (pathname === "/sw.js") {
      res.writeHead(200, { "Content-Type": "application/javascript", "Cache-Control": "no-store", "Service-Worker-Allowed": "/" });
      return res.end(fs.readFileSync(path.join(web, "public", "sw.js")));
    }
    if (pathname === "/__release-info" || pathname === "/api/release") {
      ...
      }, parseCookies(req.headers.cookie).tm_release === selected.version ? {} : { "Set-Cookie": cookie(selected.version) });
    }
