...
export default function SoftwareUpdateButton() {
  ...
  async function applyUpdate() {
    ...
    try {
      ...
      if (update.gateway) {
        ...
        const verification = await fetch("/__release-info?t=" + Date.now(), { cache: "no-store", credentials: "same-origin" });
        if (!verification.ok || (await verification.json()).selectedVersion !== selected) {
          throw new Error("The release could not be selected. Please allow site cookies and try again.");
        }
      }
      await clearApplicationCaches();
      try { localStorage.setItem("chakor-installed-release", selected); } catch {}
      const target = new URL(window.location.href);
      target.searchParams.set("__tm_release", selected);
  ...
  return <>
    <button type="button" onClick={confirm} disabled={busy} data-unseen={unseen}
      ...
      <span className="hidden rounded-md bg-[#092238]/10 px-2 py-1 text-[10px] sm:inline">{update.label.replace(/^v(\d+)\.0\.0$/, "Version $1")} Available</span>
