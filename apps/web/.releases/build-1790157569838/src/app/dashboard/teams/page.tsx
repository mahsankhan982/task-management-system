...
type User = { id:number|string; full_name:string; email:string; role:Role; team_id:number|string|null; is_active:boolean; avatar_url?:string|null };
      ...
      <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        ...
        <div className="divide-y">
          ...
          {members.map(u=><div key={String(u.id)} className="flex items-center justify-between gap-4 p-5">
            <div className="flex items-center gap-3"><UserAvatar user={u} /><div><p className="font-semibold text-slate-900">{u.full_name}</p><p className="mt-1 text-sm text-slate-500">{u.email}</p></div></div>
