...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-full w-full bg-transparent p-5 md:p-8">
      <section className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-[#A07D3D]">
            Live Workspace
          </p>
        ...
        {canManageBoards ? (
          <button type="button" onClick={() => void createBoard()} className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl border border-white/20 bg-transparent/10 px-5 text-sm font-semibold text-black shadow-lg backdrop-blur transition hover:-translate-y-0.5 hover:bg-transparent/20">
        ...
        {canJoinEmployee ? (
          <button
            ...
            className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-transparent px-5 text-sm font-semibold text-[#173B59] shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"
          >
            <UserPlus size={18} />
            Join Employee
          </button>
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {workspaces.map((workspace) => {
            ...
            <>
              <div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${
                board
                  ...
                  : "bg-[#F4E6BE] text-[#B88920]"
              }`}>
                <Icon size={21} />
              </div>

              <h2 className="mt-7 text-xl font-semibold text-black">
                {workspace.title}
              </h2>
          ...
          if (!board) {
            return (
              <div
                key={workspace.title}
                aria-disabled="true"
                className="min-h-[170px] cursor-not-allowed rounded-2xl border border-white/50 bg-transparent/75 p-6 opacity-70 shadow-lg shadow-slate-950/10"
              >
                {cardContent}
              </div>
            );
          ...
          return (
            <Link
              ...
              className="group min-h-[170px] rounded-2xl border border-[#D8C27A]/70 bg-[linear-gradient(135deg,#17364E_0%,#0E2D45_100%)] p-6 shadow-[0_16px_36px_rgba(14,45,69,0.18)] transition hover:-translate-y-1 hover:border-[#C79B3B] hover:shadow-xl"
            >
              {cardContent}
            </Link>
          );
        ...
        {boards
          ...
          .map((board) => (
            <div key={board.id} className="relative min-h-[170px] rounded-2xl border border-white/70 bg-transparent/95 p-6 shadow-lg shadow-slate-950/10 transition duration-300 hover:-translate-y-1.5 hover:border-[#D8C28E] hover:shadow-2xl">
              <Link href={`/dashboard/boards?boardId=${board.id}`} className="group block">
                <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-violet-50 text-[#173B59] transition group-hover:bg-violet-700 group-hover:text-black">
                  <Waypoints size={21} />
                </div>
                <h2 className="mt-7 pr-20 text-xl font-semibold text-black">{board.name}</h2>
                <p className="mt-2 text-sm text-slate-200">{board.description || `Open ${board.name}.`}</p>
              </Link>
              {canManageBoards && !board.is_system ? (
                <div className="absolute right-4 top-4 flex gap-1">
                  <button type="button" title="Edit board" onClick={() => void editBoard(board)} className="rounded-lg border bg-transparent p-2 text-slate-500 hover:text-[#173B59]"><Pencil size={15} /></button>
                  <button type="button" title="Delete board" onClick={() => void deleteBoard(board)} className="rounded-lg border bg-transparent p-2 text-slate-500 hover:border-red-200 hover:text-red-600"><Trash2 size={15} /></button>
                </div>
              ) : null}
            </div>
          ))}
      ...
      {showJoin && canJoinEmployee ? (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
          ...
          <div className="relative z-10 w-full max-w-xl rounded-2xl bg-transparent shadow-2xl">
            ...
            <form onSubmit={joinEmployee} className="p-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="text-sm font-semibold text-slate-700">
                  ...
                  <input
                    ...
                    className="mt-2 h-11 w-full rounded-2xl border px-3 text-sm font-normal outline-none focus:border-violet-500"
                  />
                </label>
                ...
                <label className="text-sm font-semibold text-slate-700">
                  ...
                  <select
                    ...
                    className="mt-2 h-11 w-full rounded-2xl border bg-transparent px-3 text-sm font-normal outline-none focus:border-violet-500"
                  >
              ...
              <div className="mt-6 flex items-center justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setShowJoin(false)}
                  className="h-10 rounded-2xl border px-4 text-sm font-semibold text-slate-600 hover:bg-slate-50"
                >
                  Cancel
                </button>
                ...
                <button
                  type="submit"
                  disabled={joining}
                  className="inline-flex h-10 items-center gap-2 rounded-2xl bg-violet-700 px-5 text-sm font-semibold text-black disabled:opacity-60"
                >
                  <UserPlus size={16} />
