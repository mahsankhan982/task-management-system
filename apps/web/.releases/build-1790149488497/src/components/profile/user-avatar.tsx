...
export function publishProfilePhoto(id: number | string, photo: string) {
  // Refresh already-mounted assignee avatars after a successful server save.
  updatedPhotos.set(String(id), photo);
...
export default function UserAvatar({ user, size = 36, className = "" }: {
  ...
  const photo = profileImageUrl(updatedPhoto || user.avatar_url || "");
