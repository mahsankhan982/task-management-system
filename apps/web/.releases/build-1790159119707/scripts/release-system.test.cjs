...
test('service worker never serves a captured old navigation', async () => {
  const handlers = {}, deleted = [];
  let response;
  const context = {
    self: { location: { origin: 'https://app.test' }, addEventListener: (type, handler) => { handlers[type] = handler; }, clients: { claim: async () => {} }, skipWaiting: async () => {} },
    URL,
    caches: { keys: async () => ['task-manager-old', 'unrelated'], delete: async name => deleted.push(name) },
    fetch: async (_request, options) => { assert.equal(options.cache, 'no-store'); return 'network-selected-release'; },
  };
  vm.runInNewContext(fs.readFileSync(path.join(__dirname, '../public/sw.js'), 'utf8'), context);
  await new Promise((resolve, reject) => handlers.activate({ waitUntil: promise => promise.then(resolve, reject) }));
  assert.deepEqual(deleted, ['task-manager-old']);
  handlers.fetch({ request: { method: 'GET', mode: 'navigate', url: 'https://app.test/dashboard' }, respondWith: value => { response = value; } });
  assert.equal(await response, 'network-selected-release');
});
...
test('version comparison and scoped cache cleanup', async () => {
  ...
  const context = {
    ...
    navigator: { serviceWorker: { getRegistrations: async () => [
      { active: { scriptURL: 'https://app.test/sw.js' }, update: async () => {}, unregister: async () => unregistered.push('app') },
...
test('release gateway preserves pins and discovers new builds without restart', async () => {
  ...
  const mockedRequire = name => {
    if (name === 'fs') return {
      readFileSync: () => JSON.stringify(meta), existsSync: file => !file.includes('broken'),
      writeFileSync: (_file, value) => { Object.assign(meta, JSON.parse(value)); }, renameSync() {},
    };
  ...
  try {
    ...
    assert.equal(meta.defaultVersion, 'build-new');
    assert.equal(meta.currentVersion, 'build-new');
    const pinned = await (await fetch(base + '/__release-info', { headers: { Cookie: 'tm_release=build-old' } })).json();
    assert.equal(pinned.selectedVersion, 'build-old');
