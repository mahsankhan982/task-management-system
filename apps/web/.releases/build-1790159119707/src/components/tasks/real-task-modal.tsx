...
export default function RealTaskModal({
  ...
  const canEditDueDate = canEditTaskDueDate(role);
  ...
  async function saveTask() {
    ...
    try {
      ...
      if (role === "Team Member" && permissions.assignTask && !assigneeIds.some(id => Number(id) === Number(user.id))) {
        await onChanged?.(); onClose(); return;
      }
      await loadTask();
      await onChanged?.();
      setEditing(false);
                ...
                <label className="rounded-2xl border border-rose-100 bg-rose-50/60 p-3 text-[11px] font-bold uppercase tracking-wider text-rose-700 shadow-sm">
                  ...
                  {editing && !canEditDueDate ? (
                    <span className="mt-2 block text-[10px] normal-case text-rose-600">
                      Team Members can view due dates but cannot change them.
                    </span>
                  ) : null}
                </label>
              </div>
